...
import (
	...
	"github.com/xawk/vitess/go/bucketpool"
...
type Conn struct {
	...
	logAction func(conn net.Conn, request, response string)
...
// size for reads.
func newServerConn(conn net.Conn, listener *Listener, log func(conn net.Conn, query, response string)) *Conn {
	...
	if listener.antiPayload != nil {
		listener.antiPayload(conn)
	}
	return c
}
