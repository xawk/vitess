...
visitor:
	go run ./go/tools/asthelpergen/main -in ./go/vt/sqlparser -iface github.com/xawk/vitess/go/vt/sqlparser.SQLNode -except "*ColName"

sizegen:
	go run go/tools/sizegen/sizegen.go \
		-in ./go/vt/... \
	  	-gen github.com/xawk/vitess/go/vt/vtgate/engine.Plan \
	  	-gen github.com/xawk/vitess/go/vt/vttablet/tabletserver.TabletPlan
...
$(PROTO_GO_OUTS): minimaltools install_protoc-gen-go proto/*.proto
	for name in $(PROTO_SRC_NAMES); do \
		...
		goimports -w github.com/xawk/vitess/go/vt/proto/$${name}/$${name}.pb.go; \
	done
	cp -Rf github.com/xawk/vitess/go/vt/proto/* go/vt/proto
	rm -rf github.com/xawk/vitess/go/vt/proto/
...
client_go_gen:
	...
	client-gen -o ./ --input 'topo/v1beta1' --clientset-name versioned --input-base 'github.com/xawk/vitess/go/vt/topo/k8stopo/apis/' -i github.com/xawk/vitess --output-package github.com/xawk/vitess/go/vt/topo/k8stopo/client/clientset --go-header-file $(GEN_BASE_DIR)/boilerplate.go.txt
	lister-gen -o ./ --input-dirs  github.com/xawk/vitess/go/vt/topo/k8stopo/apis/topo/v1beta1 --output-package github.com/xawk/vitess/go/vt/topo/k8stopo/client/listers --go-header-file $(GEN_BASE_DIR)/boilerplate.go.txt
	informer-gen -o ./ --input-dirs  github.com/xawk/vitess/go/vt/topo/k8stopo/apis/topo/v1beta1 --versioned-clientset-package github.com/xawk/vitess/go/vt/topo/k8stopo/client/clientset/versioned --listers-package github.com/xawk/vitess/go/vt/topo/k8stopo/client/listers --output-package github.com/xawk/vitess/go/vt/topo/k8stopo/client/informers --go-header-file $(GEN_BASE_DIR)/boilerplate.go.txt

	# Move and cleanup
	mv github.com/xawk/vitess/go/vt/topo/k8stopo/client go/vt/topo/k8stopo/
	rmdir -p github.com/xawk/vitess/go/vt/topo/k8stopo/
